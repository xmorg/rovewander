...
function create_town_map(walls)--walls boolean
   ...
   for x=2, 7 do --ensure exit is not blocked by walls
      game_map[game.tilecount/2][x] = "+"
      game_map[game.tilecount/2+1][x] = "+"
      game_map[game.tilecount/2][game.tilecount-x] = "+"
   end
   create_fog_of_war(game.tilecount)
end
...
function create_sea_map()--1-10 1 being green 10 being brown
   ...
   for x = 1, game.tilecount do
      if game_map[x][1] ~= "D" then game_map[x][1] = "l" end
      if game_map[1][x] ~= "D" then game_map[1][x] = "l" end
      if game_map[game.tilecount][x] ~= "D" then game_map[game.tilecount][x] = "l" end
      if game_map[x][game.tilecount] ~= "D" then game_map[x][game.tilecount] = "l" end
   end
   create_fog_of_war(game.tilecount)
end
